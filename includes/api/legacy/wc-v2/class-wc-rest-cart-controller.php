...
class WC_REST_Cart_Controller {
	...
	public function get_cart( $data = array() ) {
		...
		foreach ( $cart as $item_key => $cart_item ) {
			$_product = apply_filters( 'wc_cart_rest_api_cart_item_product', $cart_item['data'], $cart_item, $item_key ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
			...
			if ( $show_thumb ) {
				$thumbnail_id = apply_filters( 'wc_cart_rest_api_cart_item_thumbnail', $_product->get_image_id(), $cart_item, $item_key ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
	...
	public function add_to_cart( $data = array() ) {
		...
		if ( $product_data->is_sold_individually() ) {
			...
			$found_in_cart = apply_filters( 'woocommerce_add_to_cart_sold_individually_found_in_cart', $cart_item_key && $cart_contents[ $cart_item_key ]['quantity'] > 0, $product_id, $variation_id, $cart_item_data, $cart_id ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
		...
		if ( ! $product_data->is_in_stock() ) {
			return new WP_Error( 'wc_cart_rest_product_out_of_stock', sprintf( __( 'You cannot add &quot;%s&quot; to the cart because the product is out of stock.', 'cart-rest-api-for-woocommerce' ), $product_data->get_name() ), array( 'status' => 500 ) ); // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment
		}
		...
		if ( $product_data->managing_stock() ) {
			...
			if ( isset( $products_qty_in_cart[ $product_data->get_stock_managed_by_id() ] ) && ! $product_data->has_enough_stock( $products_qty_in_cart[ $product_data->get_stock_managed_by_id() ] + $quantity ) ) {
				return new WP_Error(
					...
					sprintf(
						__( 'You cannot add that amount to the cart &mdash; we have %1$s in stock and you already have %2$s in your cart.', 'cart-rest-api-for-woocommerce' ), // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment
		...
		if ( $item_key ) {
			...
			do_action( 'wc_cart_rest_add_to_cart', $item_key, $data ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound

			if ( is_array( $data ) ) {
				return new WP_REST_Response( $data, 200 );
			}
		} else {
			return new WP_Error( 'wc_cart_rest_cannot_add_to_cart', sprintf( __( 'You cannot add "%s" to your cart.', 'cart-rest-api-for-woocommerce' ), $product_data->get_name() ), array( 'status' => 500 ) ); // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment
		}
	} // END add_to_cart()
		...
		if ( '0' !== $cart_item_key ) {
			...
			if ( WC()->cart->set_quantity( $cart_item_key, $quantity ) ) {
				...
				if ( $quantity !== $new_data['quantity'] ) {
					do_action( 'wc_cart_rest_item_quantity_changed', $cart_item_key, $new_data ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
				}

				// Return response based on product quantity increment.
				if ( $quantity > $current_data['quantity'] ) {
					return new WP_REST_Response( sprintf( __( 'The quantity for "%1$s" has increased to "%2$s".', 'cart-rest-api-for-woocommerce' ), $product_data->get_name(), $new_data['quantity'] ), 200 ); // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment
				} elseif ( $quantity < $current_data['quantity'] ) {
					return new WP_REST_Response( sprintf( __( 'The quantity for "%1$s" has decreased to "%2$s".', 'cart-rest-api-for-woocommerce' ), $product_data->get_name(), $new_data['quantity'] ), 200 ); // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment
				} else {
					return new WP_REST_Response( sprintf( __( 'The quantity for "%s" has not changed.', 'cart-rest-api-for-woocommerce' ), $product_data->get_name() ), 200 ); // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment
				}
			} else {
