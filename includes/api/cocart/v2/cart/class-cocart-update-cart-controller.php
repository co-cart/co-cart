	...
	public function get_permissions_check( $request ) {
		...
		try {
			...
			if ( ! is_callable( array( $callback_methods[ $namespace ], 'callback' ) ) ) {
				throw new CoCart_Data_Exception( 'cocart_update_cart_invalid_callback_error', sprintf( __( 'There is no valid callback registered for: %1$s.', 'cart-rest-api-for-woocommerce' ), $namespace ), 400 );
			}
