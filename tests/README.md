...
```bash
./vendor/bin/phpunit tests/unit/class-cocart-authentication-test.php
```

### Run specific test method:
```bash
./vendor/bin/phpunit --filter test_login_endpoint_returns_user_data tests/unit/class-cocart-authentication-test.php
```
...
- `tests/framework/class-cocart-api-test-case.php` - CoCart API test case shared between versions
- `tests/framework/class-cocart-api-v1-test-case.php` - CoCart v1 API test case
- `tests/framework/class-cocart-api-v2-test-case.php` - CoCart v2 API test case
- `tests/framework/class-cocart-test-spy-rest-server.php` - REST server double for asserting on header logic

### Test Files

- `tests/unit/class-cocart-authentication-test.php` - Authentication tests
- `tests/unit/class-cocart-authentication-injection-test.php` - Authentication hook injection tests
- `tests/unit/class-cocart-logout-controller-test.php` - Logout endpoint tests
- `tests/unit/class-cocart-store-controller-test.php` - Store info endpoint tests
...
class Test_Example extends CoCart_API_V2_Test_Case {
...
#### Cart Operations
- `add_item_to_cart( $product_id, $quantity, $params )`
- `remove_item_from_cart( $item_key, $params )`
- `update_item_in_cart( $item_key, $quantity, $params )`
- `clear_cart( $params )`
- `get_cart( $params )`
- `get_cart_totals( $params )`
- `get_cart_count( $params )`
...
- `get_session_by_key( $cart_key, $key_data )` - Get specific session (requires API key)
...
- `assert_cart_has_items( $expected )`
- `assert_cart_contains_product( $product_id )`
...
The tests are designed to work with CI/CD pipelines. The `phpunit.xml.dist` file contains the configuration needed for automated testing. See `.github/workflows/phpunit-tests.yml` for the matrix this repository tests against (PHP 7.4/8.0/8.1, WordPress minimum/latest, WooCommerce minimum/latest).
