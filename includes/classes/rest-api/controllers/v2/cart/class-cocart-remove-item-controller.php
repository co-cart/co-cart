...
class CoCart_REST_Remove_Item_V2_Controller extends CoCart_REST_Cart_V2_Controller {
	...
	public function remove_item( $request = array() ) {
		try {
			...
			/**
			 * Filters the title of the item removed from the cart.
			 *
			 * @since 3.0.0 Introduced.
			 *
			 * @param string $item_removed_title Item title.
			...
			if ( empty( $current_data ) ) {
				...
				if ( isset( $removed_contents[ $item_key ] ) ) {
					...
					/**
					 * Filters the title of the item already removed from the cart.
					 *
					 * @since 3.0.0 Introduced.
					 *
					 * @param string $item_already_removed_title Item title.
					 */
