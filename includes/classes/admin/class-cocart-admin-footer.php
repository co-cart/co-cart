		...
		public function update_footer( $text ) {
			if ( isset( $_GET['page'] ) && strpos( trim( sanitize_key( wp_unslash( $_GET['page'] ) ) ), 'cocart' ) === 0 ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				...
				$changelog = sprintf(
					...
					'<a href="' . esc_url( CoCart_Helpers::build_shortlink( add_query_arg( $campaign_args, esc_url( 'https://docs.cocartapi.com/updates' ) ) ) ) . '" target="_blank" rel="noopener noreferrer">',
					'</a>'
				);
