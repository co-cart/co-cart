...
if ( ! class_exists( 'CoCart_Plugin_LiteSpeed_Cache' ) ) {
	...
	class CoCart_Plugin_LiteSpeed_Cache {
		...
		public function prevent_caching() {
			...
			$request_uri = isset( $_SERVER['REQUEST_URI'] ) ? esc_url_raw( wp_unslash( $_SERVER['REQUEST_URI'] ) ) : ''; // phpcs:disable WordPress.Security.ValidatedSanitizedInput.InputNotValidated
